...
class UploadHandlerCore extends Mediabox
{
    protected $options;
    ...
    function __construct($image_size = null, $initialize = true) {
	...
	/**
	* New method for 87CMS : in settings, resize of all images
	*/
	public static function create_scaled_image_ext($file_path, $new_file_path, $options){
		list($img_width, $img_height) = @getimagesize($file_path);
        if (!$img_width || !$img_height) {
            return false;
        }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
		...
		if ($this->validate($uploaded_file, $file, $error, $index)) {
			...
			$mediaFile = new MediaboxFile();
			$mediaFile->id_directory = $_SESSION['id_current_directory'];
			$mediaFile->filename = '';
			$mediaFile->path = '/'.date('Y').'/'.date('m').'/';
			$mediaFile->mimetype = $type;
			$mediaFile->size = round($size/1024);
			$mediaFile->add();	
			
			$orignal_file_name = $file->name;
			
			$file->name = $mediaFile->id_media.'-'.$file->name;
